...
class UploadFile extends Component {
        ...
        public function uploadNews($file, $product_id, $i) {
                if (\yii::$app->basePath . '/../uploads') {
                        ...
                        $name = $this->fileExists($path, $image_name = 'image_' . $i, $file->extension, 1);
                        $name_ = $name . '.' . $file->extension;

                        if ($file->saveAs($path . '/' . $name_)) {
                                chmod($path . '/' . $name_, 0777);
                                ...
                                Image::thumbnail($path . '/' . $name_, 870, 450)
                                        ->save($main_path . '/gallery/' . $name_, ['quality' => 80]);
                                Image::thumbnail($path . '/' . $name_, 280, 190)
                                        ->save($main_path . '/gallery_thumb/' . $name_, ['quality' => 90]);
                        }
                        return true;
                }
        }
